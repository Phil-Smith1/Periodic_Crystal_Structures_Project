...
void Min_Dist_Within_Nbhd_Cells ( vector<pair<string, double>>const& cell_shape, double **matrix, vector<Atom>const& cloud, vector<pair<double, pair<Atom, Atom>>>& min_dist )
{
    ...
    for (unsigned counter_1 = 0; counter_1 < 13; ++counter_1)
    {
        ...
        for (auto c : cloud)
        {
            ...
            {
                double distance = norm( c.cart_coords - s.cart_coords );
                
                if (distance < min_dist[0].first)
                {
                    min_dist[0] = pair<double, pair<Atom, Atom>>( distance, pair<Atom, Atom>( c, s ) );
